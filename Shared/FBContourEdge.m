...
- (instancetype) initWithBezierCurve:(FBBezierCurve *)curve contour:(FBBezierContour *)contour
{
    self = [super init];
...
{
    if ( _index == 0 )
        return (self.contour.edges)[[self.contour.edges count] - 1];
...
- (FBEdgeCrossing *) lastCrossing
{
    if ( [_crossings count] == 0 )
        return nil;
    return _crossings[[_crossings count] - 1];    
}
